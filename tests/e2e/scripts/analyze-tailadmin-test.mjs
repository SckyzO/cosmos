...
const URLS = [
  'https://demo.tailadmin.com/buttons',
  'https://demo.tailadmin.com/badge',
];
    ...
    try {
      ...
      const summary = await page.evaluate(() => {
        ...
        return {
          ...
          h3: sel('h3').map(text),
          h4: sel('h4').map(text),
          buttons: sel('button').slice(0, 80).map((el) => text(el) || '(icon)'),
          links: sel('a').slice(0, 40).map((el) => text(el)).filter(Boolean),
          inputs: sel('input,textarea,select').slice(0, 40).map((el) => ({
